...
async def get_effects_dict(tweets, client):
    ...
    for tweet in tweets:
        ...
        if snippet:
            ...
            if local_dict['effecttype'].lower() == 'yes':
                ...
                abstract_list.append({'title':tweet['Title'],'link':tweet['Link'],'side_effects':list(mapped_dict.values())})
                print(mapped_dict)
...
async def from_twitter(search_query,max_results):
